...
export function ExplodedCube({
  ...
  useLayoutEffect(() => {
    ...
    const turn = () => {
      ...
      const target = cubeRotationTarget(selected);
      gsap.to(value, {
        angle: target,
        duration: instant || media.matches ? 0 : cubeRotationDuration(
          value.angle, target, motion.values.rotationDuration,
        ),
        ease: "power1.inOut",
  ...
  }, [selected, instant, motion.values.rotationDuration]);
  useLayoutEffect(() => {
    const settle = (immediate: boolean) => {
      const value = rotation.current;
      const tween = gsap.getTweensOf(value)[0];
      if (immediate) {
        gsap.killTweensOf(value);
        value.angle = cubeRotationTarget(selected);
        renderer.current?.();
        return 0;
      }
      if (!tween) return 0;
      const remaining = tween.duration() - tween.time();
      const duration = Math.min(0.45, remaining / tween.timeScale());
      if (duration > 0) tween.timeScale(remaining / duration);
      return duration;
    };
